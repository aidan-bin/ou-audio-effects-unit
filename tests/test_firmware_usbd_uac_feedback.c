...
{
    /* 3 samples over 2 ms = 1.5 samples/ms -> 1.5 * 1024 = 1536 exactly. */
    expect_eq_u32(1536U, usbd_uac_compute_feedback_value(3U, 2U), "exact half-sample rate");

    /* 5 samples over 3 ms = 1.667 samples/ms; 5*1024 = 5120; 5120/3 = 1706.67
     * -> rounds to nearest = 1707. */
    uint32_t got = usbd_uac_compute_feedback_value(5U, 3U);
    expect_eq_u32(1707U, got, "non-exact rate rounds to nearest");
}
