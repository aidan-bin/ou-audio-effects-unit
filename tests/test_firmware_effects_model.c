...
static void test_default_slots(void)
{
    ...
    expect_eq_u8(EFFECT_TYPE_OVERDRIVE, state.slots[0], "default slot0 overdrive");
    expect_eq_u8(EFFECT_TYPE_ECHO, state.slots[1], "default slot1 echo");
    expect_eq_u8(EFFECT_TYPE_COMPRESSION, state.slots[2], "default slot2 compression");
...
static void test_normalize_clears_invalid_and_duplicate(void)
{
    ...
    expect_eq_u8(EFFECT_TYPE_OVERDRIVE, state.slots[0], "normalize keeps first overdrive");
...
static void test_assign_slot_moves_existing(void)
{
    ...
    expect_true(effects_state_assign_slot(&state, 3, EFFECT_TYPE_ECHO), "assign echo to slot3");
    expect_eq_u8(EFFECT_TYPE_ECHO, state.slots[3], "echo now in slot3");
    expect_eq_u8(EFFECT_ID_NONE, state.slots[1], "old echo slot vacated");

    expect_false(effects_state_assign_slot(&state, NUM_SLOTS, EFFECT_TYPE_OVERDRIVE),
                 "assign rejects out-of-range pos");
    expect_false(effects_state_assign_slot(&state, 0, (EffectType)99), "assign rejects invalid effect");
}
...
static void test_swap_slots_carries_enable(void)
{
    ...
    expect_eq_u8(EFFECT_TYPE_COMPRESSION, state.slots[0], "slot0 now compression");
    expect_eq_u8(EFFECT_TYPE_OVERDRIVE, state.slots[2], "slot2 now overdrive");
...
static void test_effect_enabled_reflects_slot(void)
{
    ...
    expect_true(effects_state_effect_enabled(&state, EFFECT_TYPE_OVERDRIVE), "overdrive enabled");
    expect_false(effects_state_effect_enabled(&state, EFFECT_TYPE_ECHO), "echo assigned but disabled");

    effects_state_clear_slot(&state, 1);
    expect_false(effects_state_effect_enabled(&state, EFFECT_TYPE_ECHO), "unassigned echo not enabled");
    expect_false(effects_state_effect_enabled(&state, (EffectType)99), "invalid effect not enabled");
}
...
static void test_active_effect_from_active_slot(void)
{
    ...
    EffectType active = EFFECT_TYPE_OVERDRIVE;
    expect_true(effects_state_get_active_effect(&state, &active), "active effect resolves");
    expect_eq_u8(EFFECT_TYPE_ECHO, (uint8_t)active, "active effect is echo");
...
static void test_effect_id_is_empty(void)
{
    ...
    expect_false(effect_id_is_empty((EffectId)EFFECT_TYPE_OVERDRIVE), "valid id is not empty");
}

static void test_effect_is_valid_boundaries(void)
{
    expect_true(effect_is_valid(EFFECT_TYPE_OVERDRIVE), "overdrive valid");
    expect_true(effect_is_valid(EFFECT_TYPE_COMPRESSION), "compression valid");
    expect_false(effect_is_valid((EffectType)-1), "negative effect invalid");
    expect_false(effect_is_valid((EffectType)NUM_EFFECTS), "NUM_EFFECTS out of range");
}
...
static void test_overdrive_pot_updates_target_parameter(void)
{
    ...
    // pot 1 drives overdrive.level, bounds [0, MAX_OVERDRIVE_LEVEL] from the metadata table.
    size_t expected_level = map_adc_to_param(255, 0, MAX_OVERDRIVE_LEVEL, 255);

    expect_true(effects_params_apply_pot_sample(&params, EFFECT_TYPE_OVERDRIVE, 1, 255, 255),
...
static void test_compression_unused_pots_are_noop(void)
{
    ...
    expect_true(effects_params_apply_pot_sample(&params, EFFECT_TYPE_COMPRESSION, 2, 255, 255),
                "compression pot2 noop success");
    expect_true(effects_params_apply_pot_sample(&params, EFFECT_TYPE_COMPRESSION, 3, 255, 255),
...
static void test_invalid_effect_or_index_rejected(void)
{
    ...
    expect_false(effects_params_apply_pot_sample(&params, (EffectType)99, 0, 10, 255),
                 "invalid effect rejected");
    expect_false(effects_params_apply_pot_sample(&params, EFFECT_TYPE_OVERDRIVE, 9, 10, 255),
...
static void test_effects_params_value_for_points_at_members(void)
{
    ...
    expect_true(effects_params_value_for(&params, EFFECT_TYPE_OVERDRIVE) == (const void *)&params.overdrive,
                "value_for overdrive");
    expect_true(effects_params_value_for(&params, EFFECT_TYPE_ECHO) == (const void *)&params.echo,
                "value_for echo");
    expect_true(
        effects_params_value_for(&params, EFFECT_TYPE_COMPRESSION) == (const void *)&params.compression,
        "value_for compression");
    expect_true(effects_params_value_for(&params, (EffectType)99) == NULL,
                "value_for invalid effect is NULL");
    expect_true(effects_params_value_for(NULL, EFFECT_TYPE_OVERDRIVE) == NULL, "value_for NULL params");
}
