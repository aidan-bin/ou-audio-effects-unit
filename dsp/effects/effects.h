...
typedef struct
{
    ...
    size_t gain;  // Output gain in QN, fraction of level (note: level is for consistency, keep gain
                  // at 1.0 and use level to control output)
...
typedef struct
{
    size_t delay_samples; // Number of samples of delay for last echo of an input sample (determines
                          // max pre-delay and duration of echo)
    size_t pre_delay;     // Number of samples before first echo
    size_t density;       // Discrete, evenly-spaced echoes per sample in QN (after pre-delay, before end
                          // of delay samples)
...
typedef struct
{
    size_t threshold; // Amplitude above which to apply gain reduction (NOT uint16_t value, refer to
                      // data convention)
    size_t ratio;     // Amount of gain reduction to apply in QN (0 for no compression, 1 for hard
                      // clipping)
} CompressionParam;
