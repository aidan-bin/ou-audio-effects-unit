 ...
 *	- Edge samples are those that go just before in_buf (num_taps - 1 in total), for ping-pong
 *buffering
 ...
 * 	- Assumes num_taps is a power of two (output get truncated otherwise, causing a loss of
 *gain)
...
void buf_averager(const uint16_t *in_buf, uint16_t *out_buf, const uint16_t *edge_samples_buf,
    ...
    for (ptrdiff_t n = -(ptrdiff_t)edge_samples;
         n < (ptrdiff_t)in_buf_size - (ptrdiff_t)edge_samples; n++) {
        uint32_t out = 0;
