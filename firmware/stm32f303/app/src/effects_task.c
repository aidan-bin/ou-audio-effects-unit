...
static uint32_t compute_ticks_to_wait(const EffectsTaskContext *taskContext, const EffectsTaskOps *ops)
{
    if (taskContext == NULL || ops == NULL || ops->ms_to_ticks == NULL || taskContext->sampleBufLen == 0)
    {
        return 0;
    }
...
bool effects_task_step(const EffectsTaskContext *taskContext, const EffectsTaskOps *ops)
{
    ...
    echoDelaySamplesBuf = (uint16_t *)ops->alloc(taskContext->delaySamplesLen * sizeof(uint16_t), ops->context);
    ...
    if (!ops->dma_copy(taskContext->delaySamplesBuf,
            echoDelaySamplesBuf,
            taskContext->delaySamplesLen,
            ticksToWait,
            ops->context))
    {
        processFailed = true;
    ...
    for (int i = 0; i < NUM_EFFECTS; i++)
    {
        ...
        if (effect == ECHO)
        {
            ...
            if (effects_pipeline_get_echo_delay_samples(taskContext->pipeline, &numDelaySamples) != 0)
            {
                processFailed = true;
                break;
            }
            ...
            echoInputBuf = (uint16_t *)ops->alloc((numDelaySamples + taskContext->sampleBufLen) * sizeof(uint16_t),
                ops->context);
            ...
            {
                if (!ops->dma_copy(&echoDelaySamplesBuf[taskContext->delaySamplesLen - numDelaySamples],
                        echoInputBuf,
                        numDelaySamples,
                        ticksToWait,
                        ops->context))
            ...
            if (!ops->dma_copy(inputBuf,
                    &echoInputBuf[numDelaySamples],
                    taskContext->sampleBufLen,
                    ticksToWait,
                    ops->context))
            {
