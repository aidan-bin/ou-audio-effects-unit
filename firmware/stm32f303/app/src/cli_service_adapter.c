...
{
    CliServiceAdapterContext *context = (CliServiceAdapterContext *)ctx;
    return context != NULL && context->ops.log_set_level != NULL &&
           context->ops.log_set_level(level, context->ops.context);
}

static bool service_log_set_enabled(bool enabled, void *ctx)
{
    CliServiceAdapterContext *context = (CliServiceAdapterContext *)ctx;
    return context != NULL && context->ops.log_set_enabled != NULL &&
           context->ops.log_set_enabled(enabled, context->ops.context);
}
