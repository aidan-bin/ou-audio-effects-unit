...
static I2CHandlerResult finish_message(const I2CHandlerMessage *message, bool failed,
    I2CHandlerResult result, const I2CHandlerOps *ops, void *context)
{
...
static bool message_valid(const I2CHandlerMessage *message, const I2CHandlerOps *ops, void *context)
{
    ...
    const I2CHandlerConfig *config, const I2CHandlerOps *ops, void *context)
{
    ...
    while (!ready)
    {
        ready = ops->is_device_ready(message->address, config->trials, config->blockingTimeoutMs, context);
        ...
        if (config->tryAgainDelayMs == 0 || remainingBudgetMs <= config->tryAgainDelayMs)
        {
            return finish_message(
                message, true, I2C_HANDLER_RESULT_DEVICE_NOT_READY, ops, context);
        }
    ...
    if (message->rxTxBar)
    {
        transferStarted = ops->start_receive(message->address, message->payload, message->items, context);
    }
    else
    {
        transferStarted = ops->start_transmit(message->address, message->payload, message->items, context);
    }
